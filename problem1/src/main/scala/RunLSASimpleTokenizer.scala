...
object RunLSASimpleTokenizer {
  def main(args: Array[String]): Unit = {
    ...
    val dataPath = if (args.length > 3) args(3) else "../../input/wikipedia/articles/*/*"
    ...
    // Initialize Spark
    val conf = new SparkConf().setAppName("LSA with Simple Tokenizer")
    ...
    // Process text with simple tokenization
    val processedText: RDD[(String, mutable.Seq[String])] = plainText.map {
      case (title, contents) =>
        (title, plainTextToTokens(contents))
    }
    ...
    // Paths to save model
    val modelDir = s"RunLSASimpleTokenizer/model_terms${numTerms}_k${k}"
